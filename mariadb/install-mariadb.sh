...
################################################################################
# MariaDB 버전에 따른 phpMyAdmin 권장 버전 반환
################################################################################
get_recommended_phpmyadmin_version() {
    local mariadb_version=$1
    
    # MariaDB 버전별 phpMyAdmin 호환성
    case $mariadb_version in
        10.2|10.2.*)
            echo "5.0"  # MariaDB 10.2는 phpMyAdmin 5.0 권장
            ;;
        10.3|10.3.*)
            echo "5.1"  # MariaDB 10.3은 phpMyAdmin 5.1 권장
            ;;
        10.4|10.4.*|10.5|10.5.*)
            echo "5.2"  # MariaDB 10.4-10.5는 phpMyAdmin 5.2 권장
            ;;
        10.6|10.6.*|10.7|10.7.*|10.8|10.8.*|10.9|10.9.*|10.10|10.10.*|10.11|10.11.*|11|11.*)
            echo "latest"  # MariaDB 10.6+는 최신 phpMyAdmin 사용 가능
            ;;
        latest)
            echo "latest"  # latest MariaDB는 latest phpMyAdmin
            ;;
        *)
            # 버전을 파싱하여 메이저 버전 확인
            ...
            if [ "$major_version" -ge 11 ]; then
                echo "latest"
            elif [ "$major_version" -eq 10 ] && [ "$minor_version" -ge 6 ]; then
                echo "latest"
...
################################################################################
get_user_input() {
    print_header "MariaDB + phpMyAdmin 설정 정보 입력"
    
    # MariaDB 컨테이너 이름
    echo -e "${CYAN}MariaDB 컨테이너 이름을 입력하세요 (기본값: mariadb):${NC}"
    read -p "> " MARIADB_CONTAINER
    MARIADB_CONTAINER=${MARIADB_CONTAINER:-mariadb}
    print_success "MariaDB 컨테이너 이름: $MARIADB_CONTAINER"
    ...
    # MariaDB 버전
    echo ""
    echo -e "${CYAN}MariaDB 버전을 입력하세요 (기본값: latest):${NC}"
    print_info "예: 10.11, 10.6, 10.5, 11, latest"
    read -p "> " MARIADB_VERSION
    MARIADB_VERSION=${MARIADB_VERSION:-latest}
    print_success "MariaDB 버전: $MARIADB_VERSION"
    
    # phpMyAdmin 버전 (MariaDB 버전에 따라 자동 추천)
    echo ""
    RECOMMENDED_PMA_VERSION=$(get_recommended_phpmyadmin_version "$MARIADB_VERSION")
    if [ "$RECOMMENDED_PMA_VERSION" != "latest" ]; then
        echo -e "${CYAN}phpMyAdmin 버전을 입력하세요 (기본값: ${RECOMMENDED_PMA_VERSION}, MariaDB ${MARIADB_VERSION} 호환):${NC}"
        print_info "MariaDB ${MARIADB_VERSION}에 권장되는 버전: ${RECOMMENDED_PMA_VERSION}"
    else
        echo -e "${CYAN}phpMyAdmin 버전을 입력하세요 (기본값: latest):${NC}"
    fi
    print_info "예: 5.2, 5.1, 5.0, latest"
    read -p "> " PMA_VERSION
    ...
    # MariaDB Root 비밀번호
    echo ""
    print_info "MariaDB Root 비밀번호를 설정합니다."
    echo ""
    
    while true; do
        read -sp "MariaDB Root 비밀번호: " MARIADB_ROOT_PASSWORD
        echo ""
        ...
        read -sp "MariaDB Root 비밀번호 확인: " MARIADB_ROOT_PASSWORD_CONFIRM
        echo ""
    ...
    print_success "MariaDB Root 비밀번호 설정 완료"
    ...
    print_success "데이터베이스: $MARIADB_DATABASE"
    
    # MariaDB 사용자 이름
    echo ""
    echo -e "${CYAN}MariaDB 사용자 이름을 입력하세요 (기본값: myuser):${NC}"
    read -p "> " MARIADB_USER
    MARIADB_USER=${MARIADB_USER:-myuser}
    print_success "MariaDB 사용자: $MARIADB_USER"
    
    # MariaDB 사용자 비밀번호
    echo ""
    print_info "MariaDB 사용자 비밀번호를 설정합니다."
    echo ""
    
    while true; do
        read -sp "MariaDB 사용자 비밀번호: " MARIADB_PASSWORD
        echo ""
        ...
        read -sp "MariaDB 사용자 비밀번호 확인: " MARIADB_PASSWORD_CONFIRM
        echo ""
    ...
    print_success "MariaDB 사용자 비밀번호 설정 완료"
    
    # MariaDB 포트
    echo ""
    echo -e "${CYAN}MariaDB 포트를 입력하세요 (기본값: 3306):${NC}"
    print_info "외부 접속을 허용하려면 포트를 지정하세요. 내부 접속만 사용하려면 엔터를 누르세요."
    read -p "> " MARIADB_PORT
    MARIADB_PORT=${MARIADB_PORT:-3306}
    print_success "MariaDB 포트: $MARIADB_PORT"
...
create_directories() {
    ...
    echo "  - mariadb-data/   (MariaDB 데이터 저장)"
    echo "  - mariadb-init/   (초기화 SQL 스크립트)"
}
...
generate_init_script() {
    ...
    cat > mariadb-init/00-security.sql << EOF
-- Root 계정 보안 설정
-- 생성일: $(date)
...
CREATE USER IF NOT EXISTS 'root'@'${NETWORK_SUBNET}' IDENTIFIED BY '${MARIADB_ROOT_PASSWORD}';
...
# MariaDB + phpMyAdmin
################################################################################
# MariaDB 데이터베이스 + phpMyAdmin 웹 관리 도구
#
# 접속 정보:
#   - phpMyAdmin: http://YOUR_SERVER_IP:${PMA_PORT}
#   - MariaDB 호스트: ${MARIADB_CONTAINER}:3306
#   - 사용자: ${MARIADB_USER}
#   - 데이터베이스: ${MARIADB_DATABASE}
#
# 보안 설정:
#   - Root 계정: ${NETWORK_NAME} 네트워크(${NETWORK_SUBNET})에서만 접속 가능
#   - 일반 사용자(${MARIADB_USER}): 모든 위치에서 접속 가능
#
################################################################################
...
save_configuration() {
    ...
    # 설정 정보 파일 생성 (비밀번호 제외)
    cat > .mariadb-config << EOF
# MariaDB + phpMyAdmin 설정 정보
# 생성일: $(date)
...
# MariaDB 직접 접속: mysql -h localhost -P ${MARIADB_PORT} -u ${MARIADB_USER} -p
EOF
    
    chmod 600 .mariadb-config
    
    print_success "설정 정보가 .mariadb-config 파일에 저장되었습니다."
}
...
start_service() {
    ...
    print_info "MariaDB + phpMyAdmin을 시작합니다..."
    echo ""
    ...
    echo ""
    print_info "MariaDB가 초기화되는 중입니다... (30초 대기)"
    sleep 30
    
    # 컨테이너 상태 확인
    if docker ps | grep -q $MARIADB_CONTAINER && docker ps | grep -q $PMA_CONTAINER; then
...
final_summary() {
    ...
    echo ""
    print_success "MariaDB + phpMyAdmin 설치가 완료되었습니다!"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "📋 생성된 파일 및 디렉토리"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  ✓ docker-compose.yml    (Docker Compose 설정)"
    echo "  ✓ .mariadb-config       (설정 정보 - 안전하게 보관!)"
    echo "  ✓ mariadb-data/         (MariaDB 데이터 저장소)"
    echo "  ✓ mariadb-init/         (초기화 SQL 스크립트)"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "📝 설정 정보"
    ...
    echo "  mysql -h localhost -P ${MARIADB_PORT} -u ${MARIADB_USER} -p"
    echo ""
    echo "  # 같은 네트워크 컨테이너에서 Root 접속 (내부 전용)"
    echo "  docker exec -it ${MARIADB_CONTAINER} mysql -u root -p"
    echo ""
    echo "  # 다른 컨테이너에서 Root 접속 (${NETWORK_NAME} 네트워크 내부)"
    echo "  mysql -h ${MARIADB_CONTAINER} -u root -p"
    echo ""
    echo "  # 다른 컨테이너에서 일반 사용자 접속"
    echo "  mysql -h ${MARIADB_CONTAINER} -u ${MARIADB_USER} -p"
    echo ""
    echo "  # 연결 문자열 (애플리케이션)"
    echo "  Host: ${MARIADB_CONTAINER}"
    echo "  Port: 3306"
    echo "  Database: ${MARIADB_DATABASE}"
    echo "  User: ${MARIADB_USER} (일반 작업) 또는 root (관리 작업, 네트워크 내부만)"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "🔒 보안 정보"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  ✓ Root 계정: ${NETWORK_NAME} 네트워크(${NETWORK_SUBNET})에서만 접속 가능"
    echo "  ✓ 외부에서는 일반 사용자(${MARIADB_USER})로만 접속 가능"
    ...
    echo "  • MariaDB 로그: docker compose logs -f ${MARIADB_CONTAINER}"
    echo "  • 서비스 재시작: docker compose restart"
    echo "  • 서비스 중지: docker compose down"
    echo "  • MariaDB 쉘 접속: docker exec -it ${MARIADB_CONTAINER} mysql -u root -p"
    echo "  • Root 계정 확인: docker exec -it ${MARIADB_CONTAINER} mysql -u root -p -e \"SELECT User, Host FROM mysql.user WHERE User='root';\""
    echo "  • 백업: docker exec ${MARIADB_CONTAINER} mysqldump -u root -p --all-databases > backup.sql"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "⚠️  보안 주의사항"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  🔴 다음 보안 작업을 수행하세요:"
    echo "    1. 방화벽 설정 (MariaDB 포트 외부 접근 제한 권장)"
    ...
    echo "📚 참고 문서"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  • MariaDB 공식 문서: https://mariadb.com/kb/en/"
...
main() {
    ...
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║                                                               ║"
    echo "║      MariaDB + phpMyAdmin 자동 설치 스크립트                   ║"
    echo "║       데이터베이스 + 웹 관리 도구                              ║"
